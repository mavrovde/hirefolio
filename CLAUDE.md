...
- **Subagents** (`.claude/agents/`): `devops-pipeline` (babysit CI after a push), `backend-dev`,
  `frontend-dev` (reproduce → fix → verify a given diagnosis).
...
3. **Green before push.** Run ruff/format + mypy + tests locally. Never push code that fails a gate.
...
8. **No rogue prod actions.** Deploy only via the sanctioned path (push to `main` / `release.sh`) and
   only when green. Confirm before anything irreversible or outward-facing.
