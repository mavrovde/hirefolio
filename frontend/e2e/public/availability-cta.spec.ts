...
test.describe('Availability indicator + hire-me CTA', () => {
    ...
    test('renders the indicator and the CTA on the hero', async ({ page }) => {
        ...
        await expect(indicator).toContainText(/Open to conversations|Gesprächsbereit/);
    ...
    test('flipping the state in the admin API changes the public hero', async ({ page, request, baseURL }) => {
        ...
        //
        // The API rides the SAME origin the browser uses: CI's E2E stack
        // (docker-compose.prod.yml) publishes only the proxy, which serves
        // /api on the public vhost — :8000 exists only in the dev topology
        // (#295 review round 1, measured red under the CI topology). And
        // Playwright's request.post THROWS on a refused connection rather
        // than returning !ok, so the reachability probe must catch.
