...
import { HttpXhrBackend, HttpRequest } from '@angular/common/http';
...
describe('SsrHttpBackend', () => {
  let xhrBackendSpy: { handle: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    xhrBackendSpy = { handle: vi.fn().mockReturnValue(of({} as any)) };
  });
  ...
  const createBackend = (isServer: boolean) => {
    TestBed.configureTestingModule({
      providers: [
        SsrHttpBackend,
        { provide: HttpXhrBackend, useValue: xhrBackendSpy },
  ...
  it('dispatches the request unmodified when running in the browser', () => {
    ...
    // Browser path is byte-identical to the baseline XHR dispatch (issue #94).
    expect(xhrBackendSpy.handle).toHaveBeenCalledWith(req);
  });
  ...
  it('prefixes relative /api urls with the backend container DNS on the server', () => {
    ...
    const calledReq = xhrBackendSpy.handle.mock.calls[0][0] as HttpRequest<any>;
  ...
  it('does not mutate the request url used by the (root-level) HTTP transfer cache — regression #25', () => {
    ...
    // cache key from `req.url` *before* handing off to the backend. As long as
    // this backend only rewrites the URL it forwards to `HttpXhrBackend` (and
    // never mutates the object passed in), the transfer cache sees the same
    // relative URL on the server and the client, so the client reuses the
    // SSR-cached response instead of re-fetching on hydration.
    const backend = createBackend(true);
