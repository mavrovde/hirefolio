...
* The data step is **guarded by the encryption key**: when
  ``BEACONFOLIO_GEMINI_ENCRYPTION_KEY`` is unset, ``app.services.crypto.encrypt`` returns the
...
def downgrade() -> None:
    ...
    for row in rows:
        ...
        plain = decrypt(raw)
        if plain is None:
            # decrypt() fails safe to None when BEACONFOLIO_GEMINI_ENCRYPTION_KEY is
            ...
            raise RuntimeError(
                ...
                "downgrade: BEACONFOLIO_GEMINI_ENCRYPTION_KEY is missing, rotated, "
