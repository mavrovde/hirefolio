...
``settings.gemini_encryption_key`` (env ``HIREFOLIO_GEMINI_ENCRYPTION_KEY``).
...
def _get_fernet() -> Fernet | None:
    ...
    (``encrypt``) that is intentional — a broken ``HIREFOLIO_GEMINI_ENCRYPTION_KEY`` is a
...
def decrypt(value: str | None) -> str | None:
    ...
    try:
        ...
        if fernet is None:
            logger.warning(
                "Encrypted secret found but HIREFOLIO_GEMINI_ENCRYPTION_KEY is not set; "
                "cannot decrypt — treating as unset."
            )
            return None
        return fernet.decrypt(token.encode()).decode()
    except (InvalidToken, ValueError) as exc:
        # ValueError covers a malformed HIREFOLIO_GEMINI_ENCRYPTION_KEY (raised by the
        ...
        logger.warning(
            ...
            "HIREFOLIO_GEMINI_ENCRYPTION_KEY) — treating as unset.",
            type(exc).__name__,
        )
        return None
