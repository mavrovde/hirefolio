...
def seed_posts():
    ...
    files = {
        "en": "blog_data_en.json",
        "de": "blog_data_de.json"
    }
            ...
            try:
                with urllib.request.urlopen(req) as response:
                    if response.status in [200, 201]:
                        ...
                        print(f"Failed to seed {post_data['id']} ({lang}): {response.status}")
            except urllib.error.HTTPError as e:
                # If 400/409, might already exist
                if e.code in [400, 409]:
                   print(f"Post {post_data['id']} ({lang}) might already exist: {e.code}")
                else:
                   print(f"HTTP Error seeding {post_data['id']} ({lang}): {e.code} - {e.read().decode()}")
