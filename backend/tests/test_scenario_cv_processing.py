...
async def test_scenario_cv_request_success(client: AsyncClient):
    ...
    with patch("sqlalchemy.ext.asyncio.AsyncSession.execute", side_effect=side_effect_execute):
        with patch("sqlalchemy.ext.asyncio.AsyncSession.commit", new_callable=AsyncMock):
            with patch("sqlalchemy.ext.asyncio.AsyncSession.refresh", new_callable=AsyncMock):
