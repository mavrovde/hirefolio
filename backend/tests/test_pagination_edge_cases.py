...
async def test_posts_page_size_exceeds_max(client: AsyncClient):
    ...
    response = await client.get("/api/posts?page_size=101")
    assert response.status_code == 422
...
async def test_posts_page_size_zero(client: AsyncClient):
    ...
    response = await client.get("/api/posts?page_size=0")
    assert response.status_code == 422
...
async def test_posts_invalid_sort_order(client: AsyncClient):
    ...
    response = await client.get("/api/posts?sort_order=invalid")
    assert response.status_code == 422
...
async def test_posts_empty_search_string(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/posts?search=")
...
async def test_posts_search_sql_injection_attempt(client: AsyncClient, db_session):
    ...
    # Try SQL injection
    response = await client.get("/api/posts?search=' OR '1'='1")
    assert response.status_code == 200
    data = response.json()
...
async def test_posts_unicode_search(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/posts?search=Привет")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
...
async def test_posts_exact_page_boundary(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/posts?page=3&page_size=10")
    assert response.status_code == 200
    data = response.json()
...
async def test_posts_single_item_multiple_pages(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/posts?page=1&page_size=1")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 3
...
async def test_posts_sort_with_null_values(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/posts?sort_by=summary&sort_order=asc")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 3
...
async def test_cv_requests_search_empty_fields(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/admin/cv/requests?search=john")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
...
async def test_tags_search_with_hyphen(client: AsyncClient, db_session):
    ...
    response = await client.get("/api/tags?search=next-js")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] >= 1
