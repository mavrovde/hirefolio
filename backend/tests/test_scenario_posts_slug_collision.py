...
async def test_posts_scenario_slug_collision_auto_retry(client: AsyncClient):
    ...
    with patch("app.api.posts.get_current_admin_user", return_value=user_mock):
        with patch(
            "sqlalchemy.ext.asyncio.AsyncSession.commit", side_effect=side_effect_commit
        ):
            # Use AsyncMock for async methods
            with patch(
                "sqlalchemy.ext.asyncio.AsyncSession.rollback", new_callable=MagicMock
            ) as mock_rollback:
                mock_rollback.return_value.__aenter__ = MagicMock(
                    return_value=None
