...
async def test_ai_generate_full_post_http_error():
    ...
    with patch("app.services.ai._generate_text_gemini", new_callable=AsyncMock) as mock_gemini:
...
async def test_ai_generate_full_post_missing_keys():
    ...
    with patch("app.services.ai._generate_text_gemini", new_callable=AsyncMock) as mock_gemini:
        mock_gemini.return_value = '{"title": "Missing Slug"}' # Valid JSON, missing keys
