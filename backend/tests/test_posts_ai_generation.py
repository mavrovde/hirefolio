...
@pytest.mark.asyncio
async def test_generate_post_retry_on_slug_collision(
    ...
    response = await client.post(
        f"{settings.api_prefix}/posts/generate", json={"topic": "Test AI"}
    )
...
@pytest.mark.asyncio
async def test_generate_post_ai_returns_none(
    client: AsyncClient, mock_embedding, mocker
):
