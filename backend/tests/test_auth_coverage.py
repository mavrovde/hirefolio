...
from app.services.auth import get_current_user, get_current_admin_user, create_access_token, get_current_user_optional
...
async def test_get_current_user_missing_sub():
    with patch("app.services.auth.decode_access_token", return_value={"other": "claim"}):
        db = MagicMock(spec=AsyncSession)
        with pytest.raises(HTTPException) as exc:
