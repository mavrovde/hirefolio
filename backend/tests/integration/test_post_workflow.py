...
async def test_complete_post_workflow(client: AsyncClient, mock_embedding):
    ...
    with patch("app.services.embeddings.get_embedding", return_value=mock_embedding), \
        ...
        update_response = await client.put(f"/api/posts/{post_data['slug']}", json=update_data)
        ...
        search_response = await client.get("/api/posts/search/semantic?q=ollama+guide&lang=en")
