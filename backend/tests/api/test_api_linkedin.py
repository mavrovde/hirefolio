...
def override_auth_for_all():
    app.dependency_overrides[get_current_admin_user] = lambda: User(id=1, username="admin")
    yield
...
def test_sync_linkedin_profile_success(monkeypatch):
    ...
    async def mock_sync_profile():
        return mock_data
    monkeypatch.setattr("app.api.linkedin.linkedin_service.sync_profile", mock_sync_profile)
    
    response = client.get(f"{settings.api_prefix}/linkedin/profile-sync", headers={"Authorization": "Bearer fake"})
...
def test_get_linkedin_posts_success(monkeypatch):
    ...
    async def mock_fetch_posts():
        return mock_posts
    monkeypatch.setattr("app.api.linkedin.linkedin_service.fetch_posts", mock_fetch_posts)
    
    response = client.get(f"{settings.api_prefix}/linkedin/posts", headers={"Authorization": "Bearer fake"})
    assert response.status_code == 200
...
def test_transfer_linkedin_post_success(monkeypatch):
    ...
    class MockSession:
        ...
        async def refresh(self, p): pass

    app.dependency_overrides[get_db] = lambda: MockSession()

    response = client.post(f"{settings.api_prefix}/linkedin/transfer-post", json=post_data, headers={"Authorization": "Bearer fake"})
    assert response.status_code == 200
    data = response.json()
    assert "id" in data
...
def test_transfer_linkedin_post_db_error(monkeypatch):
    ...
    class ErrorSession:
        def add(self, p): pass
        async def commit(self): raise Exception("DB Error")
