...
async def test_posts_scenario_vector_search_failure_fallback(client: AsyncClient):
    ...
    with patch("app.api.posts.get_embedding", new_callable=AsyncMock, return_value=[0.1] * 768):
        ...
        with patch("sqlalchemy.ext.asyncio.AsyncSession.execute", side_effect=side_effect):
