...
SCRATCH_DB = f"test_beaconfolio_encmig_{os.getenv('PYTEST_XDIST_WORKER', 'main')}"
...
def test_backfill_refuses_without_key(monkeypatch):
    """The backfill refuses (returns 0) when BEACONFOLIO_GEMINI_ENCRYPTION_KEY is unset."""
    from scripts.backfill_encrypt_gemini_key import backfill
