...
async def test_suggest_tags_valid_json():
    ...
    with respx.mock(base_url=settings.ollama_url) as respx_mock:
        respx_mock.post("/api/generate").mock(return_value=Response(200, json={
            "response": '["angular", "typescript", "web-dev"]'
        }))
...
async def test_suggest_tags_fallback_text():
    ...
    with respx.mock(base_url=settings.ollama_url) as respx_mock:
        ...
        respx_mock.post("/api/generate").mock(return_value=Response(200, json={
            "response": "Here are tags: rust, performance, memory-safety"
        }))
...
async def test_suggest_tags_connection_error():
    ...
    with respx.mock(base_url=settings.ollama_url) as respx_mock:
        import httpx
        respx_mock.post("/api/generate").mock(side_effect=httpx.ConnectError("Connection refused", request=None))
