...
async def test_posts(db_session: AsyncSession):
    # Create sample posts
    posts = [
        Post(title="Post 1", slug="post-1", content="Content 1", published=True, language="en", tags=["tag1"], embedding=[0.1]*768),
        Post(title="Post 2", slug="post-2", content="Content 2", published=True, language="de", tags=["tag2"], embedding=[0.2]*768),
        Post(title="Draft 1", slug="draft-1", content="Draft Content", published=False, language="en", tags=["tag1"], embedding=[0.3]*768),
    ]
...
async def test_crud_operations(client: AsyncClient, db_session: AsyncSession):
    ...
    with patch("app.api.posts.get_embedding", return_value=[0.1]*768):
        # Create
        resp = await client.post("/api/app/posts", json={
            "title": "New Post", "slug": "new-post", "content": "New Content", "tags": ["new"]
        })
        assert resp.status_code == 200
        post_id = resp.json()["id"]
        
        # Update
        resp = await client.put(f"/api/app/posts/{post_id}", json={
...
async def test_ai_endpoints(client: AsyncClient):
    ...
    with patch("app.services.ai.suggest_post_details", return_value={
        "title": "AI Title", "slug": "ai-slug", "summary": "AI Summary", "tags": ["ai"]
    }):
        resp = await client.post("/api/app/posts/suggest-details", json={"content": "Some content"})
        assert resp.status_code == 200
        assert resp.json()["title"] == "AI Title"

    # Test generate
    with patch("app.services.ai.generate_full_post", return_value={
        "title": "Gen Post", "slug": "gen-post", "content": "Gen Content", "tags": ["gen"]
    }), patch("app.api.posts.get_embedding", return_value=[0.1]*768):
        resp = await client.post("/api/app/posts/generate", json={"topic": "AI", "keywords": ["ai"]})
        assert resp.status_code == 200
...
async def test_post_retrieval(client: AsyncClient, test_posts):
    ...
    with patch("app.services.ai.suggest_tags", return_value=["ai", "ml"]):
        resp = await client.post("/api/app/posts/suggest-tags", json={"title": "AI", "content": "ML"})
        assert resp.status_code == 200
...
async def test_validation_and_errors(client: AsyncClient, test_posts):
    ...
    resp = await client.post("/api/app/posts", json={
        "title": "Test", "slug": "test", "content": "Content", "tags": ["1", "2", "3", "4", "5", "6"]
    })
