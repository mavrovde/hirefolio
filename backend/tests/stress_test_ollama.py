    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if has_self_secret and not found_others:
                ...
                print(f"User {user_id}: FAILED - Self-identifier not found in summary (Duration: {duration:.2f}s)")
                ...
                print(f"User {user_id}: CRITICAL FAILURE - CONTAMINATION DETECTED! (Duration: {duration:.2f}s)")
                print(f"  Found other secrets: {found_others}")
                print(f"  Summary produced: {summary}")
                return False
        else:
            print(f"User {user_id}: FAILED - Status {response.status_code} (Duration: {duration:.2f}s)")
            return False
