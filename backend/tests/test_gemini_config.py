...
async def test_update_gemini_key(client: AsyncClient, db_session: AsyncSession):
    ...
    response = await client.put("/api/app/auth/gemini-key", json={"api_key": new_key})
...
async def test_update_gemini_key_remove(client: AsyncClient, db_session: AsyncSession):
    ...
    # Set key first
    await client.put("/api/app/auth/gemini-key", json={"api_key": "some-key"})
...
async def test_ai_service_uses_user_key(
    ...
    # 1. Set user key
    await client.put("/api/app/auth/gemini-key", json={"api_key": user_key})
