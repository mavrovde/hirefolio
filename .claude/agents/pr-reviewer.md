...
4. **MUTATION-CHECK the tests that claim to pin a fix.** A test that passes against the *unfixed*
   ...
   worktree (`git checkout origin/main -- <file>` — note `git stash -- <file>` is a **no-op** when
   the change is already committed) and confirm the test fails; report the number ("14 passed,
   4 failed against the pre-fix script"). If every case still passes, the tests are decoration and
...
6. **Signature/behavior changes need a FULL-suite run, not `-k`.** Stale sibling tests in other
   files (an old mock arity, a patch of a deleted symbol) are invisible to a targeted run and have
   twice reddened the deploy. Ask for the full-run output.
