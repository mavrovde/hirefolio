...
4. `CLAUDE.md` (engineering rules 1–8 + the issue/label workflow) — the standards this repo holds itself to. Notable: root-cause not band-aids; **100% test coverage**; typing is law (Pydantic / explicit TS interfaces, no `any`); async backend I/O; Signals/`isPlatformBrowser()` SSR-safety on the frontend; docs + CHANGELOG `[Unreleased]` updated with code; **the repo is PUBLIC — no secrets in code/issues/PRs**.
...
## Verdict — you MUST post it as a real PR review
Decide one of:
- **APPROVE** — meets the acceptance criteria, CI green, no correctness/security/regression blocker, tests adequate. Post:
  `gh pr review <N> --approve --body "<summary>"`
- **REQUEST CHANGES** — any blocker (failing/absent CI without justification, unmet acceptance criterion, correctness or security bug, prod-deploy hazard, missing tests/regression coverage, committed secret). Post:
  `gh pr review <N> --request-changes --body "<numbered, actionable findings>"`
- Use **COMMENT** (`gh pr review <N> --comment --body ...`) only for non-blocking observations when you are not the deciding pass.
